...
Plotting the sample autocorrelation will give us some insight. The sample correlation, also known as the ACF, if defined by:
...
par(mfrow=c(1,2))
acf(coredata(sun.data), lag.max = 500, main = "ACF of Monthly Sunspots ")
pacf(coredata(sun.data), lag.max = 500, main = "PACF of Monthly Sunspots")
```
...
```{r zoomed_plot_first500, fig.cap="Last 200 observations of sample data"}
...
abline(v = index(zoomed_data)[20], col = "red", lty = 2)
abline(v = index(zoomed_data)[150], col = "red", lty = 2)
```

Over short term periods, it may appear that there are linear trends. However, looking back at the full data, it becomes apparent that these are elements of a much broader cyclical trend. There are many successive points indicating autoregression, however the strength of the relationship changes over time with a stronger relationship occurring at the low point of the cycle. This shows ut that it's obvious that the variance changes. Furthermore, figure 2 highlights the general concurrences of an approximate 11 year solar cycle. 
...
```{r}
# Plotting short and long term ACF to compare
par(mfrow=c(1,2))
acf(coredata(sun.data), lag.max = 50, main = "Long Term")
acf(coredata(zoomed_data), lag.max = 40, main = "Short Term")
```


The ACF plot on bother long and short terms sections of the data yield the same results, this implies weak stationarity, and confirms what we expected from the initial plots. This is kind of a bummer as stationarity leads to convenient statistical properties. 
